...
void jy61p_ReceiveData(uint8_t RxData)
{
	...
	else if (RxState == 1)
	{
		...
		else if (RxData == 0x53)	/*判断数据内容，修改这里可以改变要读的数据内容，0x53为角度输出*/
		{
			RxBuffer[RxIndex] = RxData;
			RxState = 2;
			RxIndex = 2;	//进入下一状态
	...
	else if (RxState == 3)
	{
		...
		if(RxIndex == 11)	//接收完成
		{
			...
			{
				/*计算数据，根据数据内容选择对应的计算公式*/
				Gx = ((int16_t) ((int16_t) RxBuffer[3] << 8 | (int16_t) RxBuffer[2])) / 32768.0f * 2000.0f;
				Gy = ((int16_t) ((int16_t) RxBuffer[5] << 8 | (int16_t) RxBuffer[4])) / 32768.0f * 2000.0f;
				Gz = ((int16_t) ((int16_t) RxBuffer[7] << 8 | (int16_t) RxBuffer[6])) / 32768.0f * 2000.0f;
			}
			RxState = 0;
			RxIndex = 0;	//读取完成，回到最初状态，等待包头
		}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		jy61p_ReceiveData(uart6_receive);
